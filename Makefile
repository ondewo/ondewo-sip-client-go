...
# =====================================================================================
# ondewo-sip-client-go - Makefile
#
# The ONDEWO SIP (SIP Telephony) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-sip-api submodule by the
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-sip-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_SIP_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_SIP_VERSION}
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO SIP Go Client ${ONDEWO_SIP_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-sip-client-go"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-sip-client-go repo
	pre-commit install
...
print_version: ## Print ONDEWO_SIP_VERSION and nothing else
	@echo "${ONDEWO_SIP_VERSION}"
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-sip-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-sip-client-go@v5.4.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_SIP_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_SIP_VERSION=${ONDEWO_SIP_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_SIP_VERSION=${ONDEWO_SIP_VERSION}"
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO SIP Go Client ${ONDEWO_SIP_VERSION}' entry"; \
