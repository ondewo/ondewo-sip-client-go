...
## Release ONDEWO SIP Go Client 5.4.0
...
* Version numbering follows the fleet rule: the client version matches the ONDEWO SIP API in
  major and minor version, so the first release is `5.4.0` rather than a `0.x`. The heading
  above and `ONDEWO_SIP_VERSION` in the `Makefile` now agree on it — they did not before, and
...
* `make test_coverage` gates the build on the coverage of the HAND-WRITTEN code (`auth/`, currently
  ...
  the generated stubs' figure (38.8%) without ever gating on it. `make check_stubs` runs
